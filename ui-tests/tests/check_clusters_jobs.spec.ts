...
test.describe('Clusters & Jobs tests', () => {
    ...
    async function navigateToClusters(page: Page) {
    ...
    test.skip('Can start, restart, and stop the cluster', async ({ page }) => {
        ...
        async function startCluster(clusterLocator: Locator) {
        ...
        async function restartCluster(clusterLocator: Locator) {
        ...
        async function stopCluster(clusterLocator: Locator) {
    ...
    async function fillLabels(page: Page) {
        await expect(page.locator('//div[@class="submit-job-label-header" and text()="Labels"]')).toBeVisible();
    ...
    async function submitJobAndValidate(page: Page, jobId: String) {
        await page.getByRole('button', { name: 'SUBMIT' }).click();
