...
def write_output_to_file(run_id, **kwargs):
    output_file_path = f"gs://us-central1-composer3-e9f34418-bucket/dataproc-output/multiple-check/output-notebooks/multiple-check_{run_id}.ipynb"
    print(output_file_path)
    kwargs['ti'].xcom_push(key='output_file_path', value=output_file_path)
    return output_file_path

def get_notebook_args(input_notebook,parameters):
    input_notebook = input_notebook
    output_notebook = 'gs://us-central1-composer4-fe041c11-bucket/dataproc-output/multiple-output/'
...
submit_pyspark_job_0 = DataprocSubmitJobOperator(
    ...
    job={
        ...
        'pyspark_job': {
            ...
            'args' : get_notebook_args(input_file,parameter)
        },
    },
    retries = 0,
...
submit_pyspark_job_1 = DataprocSubmitJobOperator(
    ...
    job={
        ...
        'pyspark_job': {
            'main_python_file_uri': 'gs://us-central1-composer3-e9f34418-bucket/dataproc-notebooks/wrapper_papermill.py',
            'args' : get_notebook_args(input_file,data["data"]["parameter"])
        },
    },
    retries = 0,
    gcp_conn_id='google_cloud_default',  # Reference to the GCP connection
...
submit_pyspark_job_0 >> submit_pyspark_job_1 >> submit_pyspark_job_2
