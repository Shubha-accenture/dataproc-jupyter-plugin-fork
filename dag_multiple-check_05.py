...
def write_output_to_file(run_id,task_id, **kwargs):
    output_file_path = f"gs://us-central1-composer3-e9f34418-bucket/dataproc-output/multiple-check/output-notebooks/multiple-check/{run_id}{task_id}.ipynb"
    print(output_file_path)
    kwargs['ti'].xcom_push(key='output_file_path', value=output_file_path)
    return output_file_path

def get_notebook_args(input_notebook,parameters,task_id):
    input_notebook = input_notebook
    print(task_id)
    output_notebook = "{{ ti.xcom_pull(task_ids='"+task_id+"',key='output_file_path') }}"
    print("output notebook path ---------", output_notebook)
    notebook_args = [input_notebook, output_notebook]
    parameters_str = '''
    {}
    '''.format(',\n'.join(parameters)).replace('\n', '')
